...
func TestHaveBlock(t *testing.T) {
	...
	_, isOrphan, err := chain.ProcessBlock(monautil.NewBlock(&Block1000000),
		BFNone)
	if err != nil {
...
func TestCalcSequenceLock(t *testing.T) {
	...
	// age of 4 blocks.
	targetTx := monautil.NewTx(&wire.MsgTx{
		TxOut: []*wire.TxOut{{
			PkScript: nil,
			Value:    10,
		}},
	...
	utxoView.AddTxOuts(monautil.NewTx(unConfTx), 0x7fffffff)
