...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: monautil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := monautil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*monautil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, monautil.NewTx(tx))
	}
...
func additionalCoinbase(amount monautil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee monautil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee monautil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee monautil.Amount) *wire.MsgTx {
