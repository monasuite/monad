...
func IsCoinBase(tx *monautil.Tx) bool {
...
func IsFinalizedTransaction(tx *monautil.Tx, blockHeight int32, blockTime time.Time) bool {
	msgTx := tx.MsgTx()
...
func CheckTransactionSanity(tx *monautil.Tx) error {
	...
	for _, txOut := range msgTx.TxOut {
		...
		if satoshi > monautil.MaxSatoshi {
		...
		if totalSatoshi > monautil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
// txscript.
func CountSigOps(tx *monautil.Tx) int {
	msgTx := tx.MsgTx()
...
func CountP2SHSigOps(tx *monautil.Tx, isCoinBaseTx bool, utxoView *UtxoViewpoint) (int, error) {
...
func ExtractCoinbaseHeight(coinbaseTx *monautil.Tx) (int32, error) {
...
func checkSerializedHeight(coinbaseTx *monautil.Tx, wantHeight int32) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkBlockContext(block *monautil.Block, prevNode *blockNode, flags BehaviorFlags) error {
...
func CheckTransactionInputs(tx *monautil.Tx, txHeight int32, utxoView *UtxoViewpoint, chainParams *chaincfg.Params) (int64, error) {
	...
	for txInIndex, txIn := range tx.MsgTx().TxIn {
		...
		if originTxSatoshi < 0 {
			str := fmt.Sprintf("transaction output has negative "+
				"value of %v", monautil.Amount(originTxSatoshi))
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		if originTxSatoshi > monautil.MaxSatoshi {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v",
				monautil.Amount(originTxSatoshi),
				monautil.MaxSatoshi)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		...
		if totalSatoshiIn < lastSatoshiIn ||
			totalSatoshiIn > monautil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkConnectBlock(node *blockNode, block *monautil.Block, view *UtxoViewpoint, stxos *[]SpentTxOut) error {
...
func (b *BlockChain) CheckConnectBlockTemplate(block *monautil.Block) error {
	b.chainLock.Lock()
