...
// arguments returns an array of arguments that be used to launch the monad
// process.
...
// command returns the exec.Cmd which will be used to start the monad process.
...
// to the monad process that is launched via Start().
...
// as the base for the log and data directories for monad.
...
// start creates a new monad process, and writes its pid in a file reserved for
...
// stop interrupts the running monad process process, and waits until it exits
...
// shutdown terminates the running monad process, and cleans up all
...
func genCertPair(certFile, keyFile string) error {
	...
	cert, key, err := monautil.NewTLSCertPair(org, validUntil, nil)
	if err != nil {
		return err
	}
