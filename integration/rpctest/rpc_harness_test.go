...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt monautil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(monautil.Amount(5 * monautil.SatoshiPerBitcoin))
	...
	txid = genSpend(monautil.Amount(500 * monautil.SatoshiPerBitcoin))
	blockHashes, err = r.Client.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(monautil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*monautil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, monautil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := monautil.Amount(250 * monautil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = monautil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := monautil.Amount(50 * monautil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := monautil.Amount(numMatureOutputs * 50 * monautil.SatoshiPerBitcoin)
