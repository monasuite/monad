...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) checkPoolDoubleSpend(tx *monautil.Tx) (bool, error) {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) signalsReplacement(tx *monautil.Tx,
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) txAncestors(tx *monautil.Tx,
	cache map[chainhash.Hash]map[chainhash.Hash]*monautil.Tx) map[chainhash.Hash]*monautil.Tx {

	// If a cache was not provided, we'll initialize one now to use for the
	// recursive calls.
	if cache == nil {
		cache = make(map[chainhash.Hash]map[chainhash.Hash]*monautil.Tx)
	}

	ancestors := make(map[chainhash.Hash]*monautil.Tx)
	for _, txIn := range tx.MsgTx().TxIn {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) txDescendants(tx *monautil.Tx,
	cache map[chainhash.Hash]map[chainhash.Hash]*monautil.Tx) map[chainhash.Hash]*monautil.Tx {
	...
	descendants := make(map[chainhash.Hash]*monautil.Tx)
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) txConflicts(tx *monautil.Tx) map[chainhash.Hash]*monautil.Tx {
	conflicts := make(map[chainhash.Hash]*monautil.Tx)
	for _, txIn := range tx.MsgTx().TxIn {
		conflict, ok := mp.outpoints[txIn.PreviousOutPoint]
		if !ok {
			continue
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) validateReplacement(tx *monautil.Tx,
	txFee int64) (map[chainhash.Hash]*monautil.Tx, error) {
...
func (mp *TxPool) maybeAcceptTransaction(tx *monautil.Tx, isNew, rateLimit, rejectDupOrphans bool) ([]*chainhash.Hash, *TxDesc, error) {
	...
	var conflicts map[chainhash.Hash]*monautil.Tx
	if isReplacement {
