...
const (
	...
	DefaultMinRelayTxFee = monautil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee monautil.Amount) int64 {
	...
	if minFee < 0 || minFee > monautil.MaxSatoshi {
		minFee = monautil.MaxSatoshi
	}
...
func checkInputsStandard(tx *monautil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee monautil.Amount) bool {
...
func checkTransactionStandard(tx *monautil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee monautil.Amount,
...
func GetTxVirtualSize(tx *monautil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
