...
const (
	...
	MinHighPriority = monautil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr monautil.Address) (*monautil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *monautil.Tx, height int32) error {
...
func logSkippedDeps(tx *monautil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress monautil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*monautil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := monautil.NewTx(coinbaseTx.MsgTx().Copy())
...
func AddWitnessCommitment(coinbaseTx *monautil.Tx,
	blockTxns []*monautil.Tx) []byte {
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A monautil.Block should use saved in the state to avoid
