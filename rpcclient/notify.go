...
type NotificationHandlers struct {
	...
	// result of a rescan request, due to how monad may send various rescan
	...
	// made to register for the notification and the function is non-nil.
	OnTxAccepted func(hash *chainhash.Hash, amount monautil.Amount)
	...
	// such as btcwallet.
	OnAccountBalance func(account string, balance monautil.Amount, confirmed bool)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	case btcjson.BtcdConnectedNtfnMethod:
		...
		if err != nil {
			log.Warnf("Received invalid monad connected "+
				"notification: %v", err)
			return
		}
...
//
// NOTE: This is a monad extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func parseFilteredBlockConnectedParams(params []json.RawMessage) (int32,
	*wire.BlockHeader, []*monautil.Tx, error) {
	...
	transactions := make([]*monautil.Tx, len(hexTransactions))
	for i, hexTx := range hexTransactions {
		...
		transactions[i], err = monautil.NewTxFromBytes(transaction)
		if err != nil {
			return 0, nil, nil, err
		}
	}
...
// notifications.
func parseChainTxNtfnParams(params []json.RawMessage) (*monautil.Tx,
	*btcjson.BlockDetails, error) {
	...
	return monautil.NewTx(&msgTx), block, nil
}
...
// parseBtcdConnectedNtfnParams parses out the connection status of monad
// and btcwallet from the parameters of a monadconnected notification.
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance monautil.Amount, confirmed bool, err error) {
...
func (c *Client) RescanEndBlockAsync(startBlock *chainhash.Hash,
	addresses []monautil.Address, outpoints []*wire.OutPoint,
...
// NOTE: This is a monad extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilterAsync(reload bool, addresses []monautil.Address,
...
// NOTE: This is a monad extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilter(reload bool, addresses []monautil.Address, outPoints []wire.OutPoint) error {
