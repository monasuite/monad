...
var helpDescsEnUS = map[string]string{
	...
	"getblock-verbosetx":   "Specifies that each transaction is returned as a JSON object and only applies if the verbose flag is true (monad extension)",
	...
	"getblocktemplateresult-rules":                      "List of rules of blockchain proposal",
	...
	"sendrawtransaction-allowhighfees": "Whether or not to allow insanely high fees (monad does not yet implement this parameter, so it has no effect)",
	...
	// NotifySpentCmd help.
	"notifyspent--synopsis": "Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this monad instance) and when such a transaction first appears in a newly-attached block.",
