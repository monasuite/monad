...
const (
	// svcName is the name of monad service.
	svcName = "monadsvc"
...
// logServiceStartOfDay logs information about monad when the main server has
...
// monadService houses the main service handler which handles all service
// updates and launching monadMain.
type monadService struct{}
...
// long-running monadMain (which is the real meat of monad), handles service
// change requests, and notifies the service control manager of changes.
func (s *monadService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start monadMain in a separate goroutine so the service can start
...
// installService attempts to install the monad service.  Typically this should
...
// removeService attempts to uninstall the monad service.  Typically this should
...
// startService attempts to start the monad service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &monadService{})
	if err != nil {
